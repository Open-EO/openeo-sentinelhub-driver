const http = require('http');

module.exports = function (server_storage) {
    var module = {};
    const jobs = require('./jobs')(server_storage);
    
    module.wcs = function (req, res, next) {
        const job_id = req.params.job_id;
        const job = new jobs.job_data(server_storage.get(jobs.createJobCacheKey(job_id)));
        const job_script = new Buffer(job.generateScript()).toString('base64');

        console.log("\n\n"+"Y29uc3QgZmluZE1pbkluZGV4ID0gYXJyID0%%%2BIGluZGV4KHMuQjA4LCBzLkIwNCkpOwogIHNhbXBsZXMgPSBbZmluZE1pbihzYW1wbGVzKV07CgogIHJldHVybiBzYW1wbGVzOwp9");
        console.log("\n\n"+job_script);

        http.get("http://services.sentinel-hub.com/v1/wms/b7b5e3ef-5a40-4e2a-9fd3-75ca2b81cb32?"+
            "service=WMS&request=GetMap&version=1.1.1&layers=&styles=&format=image%2Fjpeg&height=512&width=512&srs=EPSG%3A3857"+
            "&bbox=1252344.2714243277,5635549.221409476,1408887.3053523689,5792092.255337515" +
            "&TEMPORAL=true&time=2015-01-01%2F2017-01-31"+
            "&evalsource=S2&PREVIEW=3"+
            "&evalscript="+encodeURIComponent(job_script)
            , serverResponse => {
                const responseHeaders = serverResponse.headers;
                responseHeaders["job_id"] = job_id;
                responseHeaders["job_script"] = JSON.stringify(job_script);
                res.writeHeader(serverResponse.statusCode, responseHeaders);
                serverResponse.pipe(res);
            });
        return next();
    };

    return module;
}